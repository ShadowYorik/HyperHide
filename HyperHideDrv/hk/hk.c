...
)
{
	...
	TrampolineInfo.pTarget = TargetFunction;
	TrampolineInfo.pDetour = Hook;
	TrampolineInfo.pTrampoline = Trampoline + INTERLOCKED_EXCHANGE_SIZE;
	if (!CreateTrampolineFunction(&TrampolineInfo))
	{
	...
	{
		*OriginalTrampoline = TrampolineInfo.pTrampoline;
	}
